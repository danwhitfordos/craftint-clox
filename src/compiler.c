...
typedef enum { TYPE_FUNCTION, TYPE_SCRIPT, TYPE_METHOD, TYPE_INITIALISER } FunctionType;
...
static void errorAt(Token *token, const char *message) {
    ...
    fprintf(errfile, "[line %d] Error", token->line);

    if (token->type == TOKEN_EOF) {
        fprintf(errfile, " at end");
    } else if (token->type == TOKEN_ERROR) {
        // Nothing (yet?)
    } else {
        fprintf(errfile, " at '%.*s'", token->length, token->start);
    }
...
static void super_(bool canAssign) {
    ...
    if (currentClass == NULL) {
        ...
        error("Can't us 'super' in a class with no superclass.");
    }
...
ObjFunction *compile(const char *source, FILE *err) {
    initScanner(source);
    Compiler compiler;
